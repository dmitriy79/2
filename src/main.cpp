...
// coinvest, 구분자
const string strMessageMagic = "DoveCoin Signed Message:\n";
...
int64_t GetProofOfWorkReward(int64_t nFees)
{
    ...
    else if (pindexBest->nHeight+1 == 4)
    {   // coinvest, 최초 블럭 보상
        nSubsidy = 16000000000 * COIN;
        return nSubsidy;
    }
    else if (pindexBest->nHeight+1 == 5)
    {   // coinvest, 최초 블럭 보상
        nSubsidy = 16000000000 * COIN;
        return nSubsidy;
    }
    else if (pindexBest->nHeight+1 == 6)
    {   // coinvest, 최초 블럭 보상
        nSubsidy = 16000000000 * COIN;
        return nSubsidy;
    }
    else if (pindexBest->nHeight+1 == 7)
    {   // coinvest, 최초 블럭 보상
        nSubsidy = 16000000000 * COIN;
        return nSubsidy;
    }
    else if (pindexBest->nHeight+1 == 8)
    {   // coinvest, 최초 블럭 보상
        nSubsidy = 16000000000 * COIN;
        return nSubsidy;
    }
    else if (pindexBest->nHeight+1 == 9)
    {   // coinvest, 최초 블럭 보상
        nSubsidy = 16000000000 * COIN;
        return nSubsidy;
    }
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "DoveCoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "2016-10-03T13:08:20+00:00 ";
        CTransaction txNew;
        // coinvest, 최초 시작 시간
        txNew.nTime = 1475500101;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        // coinvest, 최초 시작 시간
        block.nTime    = 1475500101;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        // coinvest, nNonce
        block.nNonce   = 2833479;
        if(fTestNet)
        {
            block.nNonce   = 0;
        }
        ...
        //// debug print
        // coinvest, MerkleRoot
        assert(block.hashMerkleRoot == uint256("0x668cbd632981fe90f94c66e8a545d3ea8a679cde76181d6a609cfab48474a289"));
        block.print();
...
// coinvest, 메세지 스타트
unsigned char pchMessageStart[4] = { 0x53, 0x61, 0x66, 0x65 };
